(function attachSidepanelCustomEmailPoolManager(globalScope) {
  function createCustomEmailPoolManager(context = {}) {
    ...
    const allowedEmailDomain = String(constants.allowedEmailDomain || '').trim().toLowerCase();
    ...
    function isAllowedEmail(value = '') {
      ...
      return !allowedEmailDomain || email.endsWith(`@${allowedEmailDomain}`);
    }
    ...
    async function importEntriesFromTextarea() {
      ...
      for (const line of String(text || '').split(/[\r\n,，;；]+/)) {
        ...
        if (!isValidEmail(email) || !isAllowedEmail(email) || knownEmails.has(email)) {
          skippedCount += 1;
          continue;
        }
