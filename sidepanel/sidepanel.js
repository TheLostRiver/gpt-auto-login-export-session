...
let currentPlusAccountAccessStrategy = PLUS_ACCOUNT_ACCESS_STRATEGY_CPA_CODEX_SESSION;
...
function forceKimiCpaReloginState(state = {}) {
  if (window.KimiCpaDefaults?.applyKimiCpaReloginSettingsPatch) {
    return window.KimiCpaDefaults.applyKimiCpaReloginSettingsPatch(state);
  }
  return {
    ...(state || {}),
    ...getKimiCpaReloginSettingsPatch(),
  };
}
...
function normalizeCustomEmailPoolEntries(value = '') {
  const source = Array.isArray(value)
    ? value
    : String(value || '').split(/[\r\n,，;；]+/);
  const allowedDomain = String(KIMITOOL_ALLOWED_EMAIL_DOMAIN || '').trim().toLowerCase();

  return source
    .map((item) => String(item || '').trim().toLowerCase())
    .filter((item) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(item))
    .filter((item) => !allowedDomain || item.endsWith(`@${allowedDomain}`));
}
  ...
  const normalizePanelModeSafe = typeof normalizePanelMode === 'function'
    ...
    : ((value = '') => {
      ...
      return normalized === 'local-cpa-json'
        ...
        || isSessionTokenExportPanelMode(normalized)
        || normalized === 'sub2api'
...
function getAccountAccessStrategyUiValueForState(state = latestState) {
  ...
  if (isCpaReloginExportPanelMode(panelMode) && strategy === PLUS_ACCOUNT_ACCESS_STRATEGY_CPA_CODEX_SESSION) {
    return ACCOUNT_ACCESS_STRATEGY_UI_SESSION_JSON;
  }
...
function resolvePlusAccountAccessStrategyFromExportAndStrategy(exportTarget = '', strategyUiValue = '') {
  ...
  if (isCpaReloginExportPanelMode(target)) {
    return PLUS_ACCOUNT_ACCESS_STRATEGY_CPA_CODEX_SESSION;
  }
  return PLUS_ACCOUNT_ACCESS_STRATEGY_OAUTH;
}
...
async function restoreState() {
  try {
    let state = await chrome.runtime.sendMessage({ type: 'GET_STATE', source: 'sidepanel' });
    const forcedPatch = getKimiCpaReloginSettingsPatch();
    const needsModeFix = Object.entries(forcedPatch).some(([key, value]) => {
      if (Array.isArray(value)) {
        return JSON.stringify(state?.[key] || []) !== JSON.stringify(value);
      }
    ...
    if (needsModeFix) {
      ...
      state = forceKimiCpaReloginState(state);
    }
...
function updatePanelModeUI() {
  ...
  if (selectAccountAccessStrategy) {
    ...
    const lockStrategy = exportTarget === 'codex2api' || isCpaReloginExportPanelMode(exportTarget);
  ...
  const useSessionTokenExport = isSessionTokenExportPanelMode(panelMode);
  const useLocalFileExport = useLocalCpaJson || useSessionTokenExport;
  const useSub2Api = panelMode === 'sub2api';
  const useCodex2Api = panelMode === 'codex2api';
  const useCpa = panelMode === 'cpa';
  const useCpaRelogin = useCpa || useSessionTokenExport;
  if (selectPanelMode) {
    selectPanelMode.disabled = false;
  }
  if (useCpa) {
    if (selectPanelMode) {
      selectPanelMode.value = 'cpa';
      Array.from(selectPanelMode.options || []).forEach((option) => {
        const optionMode = getExportTargetForPanelMode(option.value || '');
        const enabled = supportedPanelModes.length
          ? supportedPanelModes.includes(optionMode)
          : true;
        option.hidden = !enabled;
        option.disabled = !enabled;
      });
  ...
  if (useCpaRelogin) {
    if (selectAccountAccessStrategy) {
      ...
      selectAccountAccessStrategy.disabled = true;
    }
    if (rowAccountAccessStrategy) {
  ...
  setRowDisplay(rowLocalCpaJsonPluginDir, useLocalFileExport);
  setRowDisplay(rowLocalCpaJsonAdvancedToggle, useLocalFileExport);
  if (!useLocalFileExport) {
  ...
  setRowDisplay(rowVpsUrl, useCpaRelogin);
  setRowDisplay(rowVpsPassword, useCpaRelogin);
...
const customEmailPoolManager = window.SidepanelCustomEmailPoolManager?.createCustomEmailPoolManager({
  ...
  constants: {
    copyIcon: COPY_ICON,
    allowedEmailDomain: KIMITOOL_ALLOWED_EMAIL_DOMAIN,
  },
});
...
function updateLocalCpaJsonAuthDirUI(expanded = localCpaJsonAuthDirExpanded, mode = null) {
  ...
  const isLocalMode = currentMode === LOCAL_CPA_JSON_PANEL_MODE
    || currentMode === LOCAL_CPA_JSON_NO_RT_PANEL_MODE
    || isSessionTokenExportPanelMode(currentMode);
...
function validateLocalCpaJsonPluginDir(options = {}) {
  ...
  const localCpaJsonNoRtMode = typeof LOCAL_CPA_JSON_NO_RT_PANEL_MODE === 'string'
    ? LOCAL_CPA_JSON_NO_RT_PANEL_MODE
    : 'local-cpa-json-no-rt';
  const required = panelMode === localCpaJsonMode
    || panelMode === localCpaJsonNoRtMode
    || isSessionTokenExportPanelMode(panelMode);
...
const baseRenderContributionMode = contributionModeManager?.render
  || (() => { });
let contributionModeRenderInFlight = false;
const renderContributionMode = () => {
  if (contributionModeRenderInFlight) {
    return;
  }
  contributionModeRenderInFlight = true;
  try {
    baseRenderContributionMode();
    renderContributionUpdateHint();
  } finally {
    contributionModeRenderInFlight = false;
  }
};
