(function attachMultiPageFlowCapabilities(root, factory) {
  ...
  const VALID_PANEL_MODES = Object.freeze([
    'local-cpa-json',
    LOCAL_CPA_JSON_NO_RT_PANEL_MODE,
    'cpa',
    ...SESSION_TOKEN_EXPORT_PANEL_MODES,
    'sub2api',
    'codex2api',
  ]);
  ...
  const PANEL_CAPABILITIES = Object.freeze({
    ...
    [ACCOUNT_TOKEN_PANEL_MODE]: Object.freeze({
      supportsPhoneSignup: true,
      requiresPhoneSignupWarning: true,
      supportedPlusAccountAccessStrategies: Object.freeze([
        PLUS_ACCOUNT_ACCESS_STRATEGY_CPA_CODEX_SESSION,
    ...
    [ACCESS_TOKEN_PANEL_MODE]: Object.freeze({
      supportsPhoneSignup: true,
      requiresPhoneSignupWarning: true,
      supportedPlusAccountAccessStrategies: Object.freeze([
        PLUS_ACCOUNT_ACCESS_STRATEGY_CPA_CODEX_SESSION,
    ...
    [SESSION_TOKEN_BUNDLE_PANEL_MODE]: Object.freeze({
      supportsPhoneSignup: true,
      requiresPhoneSignupWarning: true,
      supportedPlusAccountAccessStrategies: Object.freeze([
        PLUS_ACCOUNT_ACCESS_STRATEGY_CPA_CODEX_SESSION,
  ...
  function getPlusAccountSessionStrategyForPanel(panelMode = '') {
    ...
    if (normalizedPanelMode === 'cpa' || SESSION_TOKEN_EXPORT_PANEL_MODES.includes(normalizedPanelMode)) {
      return PLUS_ACCOUNT_ACCESS_STRATEGY_CPA_CODEX_SESSION;
    }
    return PLUS_ACCOUNT_ACCESS_STRATEGY_OAUTH;
  }
  ...
  function getPanelModeLabel(panelMode = '') {
    ...
    if (normalized === ACCOUNT_TOKEN_PANEL_MODE) {
      return 'accountToken';
    }
    if (normalized === ACCESS_TOKEN_PANEL_MODE) {
      return 'accesToken';
    }
    if (normalized === SESSION_TOKEN_BUNDLE_PANEL_MODE) {
  ...
  function createFlowCapabilityRegistry(deps = {}) {
    ...
    function resolveSidepanelCapabilities(options = {}) {
      ...
      const canUseSessionImportWithoutPlus = effectivePanelMode === 'cpa'
        || effectivePanelMode === LOCAL_CPA_JSON_NO_RT_PANEL_MODE
        || SESSION_TOKEN_EXPORT_PANEL_MODES.includes(effectivePanelMode);
      const availablePlusAccountAccessStrategies = activeFlowId === 'openai'
        && Boolean(flowState.supportsPlusMode)
        && (Boolean(runtimeLocks.plusModeEnabled) || canUseSessionImportWithoutPlus)
      ...
      const effectivePlusAccountAccessStrategy = availablePlusAccountAccessStrategies.includes(requestedPlusAccountAccessStrategy)
        ...
        : (availablePlusAccountAccessStrategies[0] || PLUS_ACCOUNT_ACCESS_STRATEGY_OAUTH);
