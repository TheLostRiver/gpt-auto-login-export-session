(function attachStepDefinitions(root, factory) {
  ...
  const CPA_SESSION_LOGIN_STEP_DEFINITIONS = [
    { id: 1, order: 10, key: 'cpa-relogin-prepare', title: '读取 CPA 401 并筛封禁邮件', sourceId: 'vps-panel', driverId: 'background/cpa-relogin-queue', command: 'cpa-relogin-prepare' },
    { id: 2, order: 20, key: 'open-chatgpt', title: '打开 ChatGPT 登录页', sourceId: 'chatgpt', driverId: null, command: 'open-chatgpt' },
    { id: 3, order: 30, key: 'chatgpt-web-login', title: '按队列邮箱登录 ChatGPT', sourceId: 'openai-auth', driverId: 'content/signup-page', command: 'chatgpt-web-login' },
    { id: 4, order: 40, key: 'fetch-login-code', title: '读取邮箱验证码并提交', sourceId: 'openai-auth', driverId: 'content/signup-page', command: 'submit-verification-code', mailRuleId: 'openai-login-code' },
    { id: 5, order: 50, key: 'cpa-session-import', title: 'SESSION 转 JSON 自动导入 CPA', sourceId: 'vps-panel', driverId: 'background/cpa-session-import', command: 'cpa-session-import' },
  ];
  const SESSION_TOKEN_EXPORT_STEP_DEFINITION = {
    id: 5,
    order: 50,
    key: 'session-token-export',
    title: '导出账号 Token 文件',
    sourceId: 'chatgpt',
  ...
  function getOpenAiResolvedStepTitle(step = {}, options = {}) {
    ...
    if (step.key === 'session-token-export') {
      const panelMode = String(options?.panelMode || '').trim().toLowerCase();
      if (panelMode === ACCOUNT_TOKEN_PANEL_MODE) {
        return '导出 accountToken 文件';
      }
      if (panelMode === ACCESS_TOKEN_PANEL_MODE) {
        return '导出 accesToken 文件';
      }
      if (panelMode === SESSION_TOKEN_BUNDLE_PANEL_MODE) {
        return '同时导出 accountToken 和 accesToken';
      }
      return step.title;
    }
    const signupMethod = getResolvedSignupMethod(options);
