...
importScripts(
  ...
  'background/cpa-relogin-queue.js',
  'background/login-config-email-list.js',
  ...
  'background/steps/chatgpt-web-login.js',
...
const KIMITOOL_ALLOWED_EMAIL_DOMAIN = '';
const KIMITOOL_DEFAULT_CPA_URL = '';
const KIMITOOL_DEFAULT_CPA_TOKEN = '';
const KIMITOOL_DEFAULT_MAIL_API_URL = '';
const KIMITOOL_DEFAULT_MAIL_ADMIN_AUTH = '';
const KIMITOOL_RELOGIN_EMAIL_CONFIG_PATH = 'data/kimi-cpa-relogin-emails.txt';
...
const loginConfigEmailReader = self.MultiPageLoginConfigEmailList?.createLoginConfigEmailListReader?.({
  chrome,
  fetch: (...args) => fetch(...args),
  allowedDomain: KIMITOOL_ALLOWED_EMAIL_DOMAIN,
  configPath: KIMITOOL_RELOGIN_EMAIL_CONFIG_PATH,
}) || null;
...
function getAuthChainStartStepId(state = {}) {
  if (isCpaReloginPanelMode(getPanelMode(state))) {
    const chatGptWebLoginStepId = typeof getStepIdByKeyForState === 'function'
      ? getStepIdByKeyForState('chatgpt-web-login', state)
      : null;
    if (Number.isInteger(chatGptWebLoginStepId) && chatGptWebLoginStepId > 0) {
      return chatGptWebLoginStepId;
    }
  }
...
const PERSISTED_SETTING_DEFAULTS = {
  ...
  vpsPassword: KIMITOOL_DEFAULT_CPA_TOKEN,
  ...
  cloudflareDomain: '',
  cloudflareDomains: [],
  cloudflareTempEmailBaseUrl: KIMITOOL_DEFAULT_MAIL_API_URL,
  cloudflareTempEmailAdminAuth: KIMITOOL_DEFAULT_MAIL_ADMIN_AUTH,
  ...
  cloudflareTempEmailDomain: KIMITOOL_ALLOWED_EMAIL_DOMAIN,
  cloudflareTempEmailDomains: KIMITOOL_ALLOWED_EMAIL_DOMAIN ? [KIMITOOL_ALLOWED_EMAIL_DOMAIN] : [],
  cloudMailBaseUrl: '',
  cloudMailAdminEmail: '',
...
const DEFAULT_STATE = {
  ...
  loginConfigEmailQueue: [],
  loginConfigEmailQueueSourcePath: KIMITOOL_RELOGIN_EMAIL_CONFIG_PATH,
  loginConfigEmailQueueUpdatedAt: 0,
  cpaReloginQueue: [],
  cpaReloginQueueCounts: {},
  cpaReloginQueueSource: '',
  cpaReloginQueueUpdatedAt: 0,
...
function applyKimiSessionGateDefaults(state = {}) {
  const normalizedDomains = normalizeCloudflareTempEmailDomains(state.cloudflareTempEmailDomains);
  const forcedSettings = self.KimiCpaDefaults?.buildKimiCpaReloginSettingsPatch?.(state) || {
    panelMode: isCpaReloginPanelMode(state?.panelMode) ? String(state.panelMode).trim().toLowerCase() : 'cpa',
    plusModeEnabled: false,
    plusAccountAccessStrategy: PLUS_ACCOUNT_ACCESS_STRATEGY_CPA_CODEX_SESSION,
  ...
  return {
    ...(state || {}),
    ...forcedSettings,
    vpsUrl: String(state?.vpsUrl || '').trim() || KIMITOOL_DEFAULT_CPA_URL,
    vpsPassword: String(state?.vpsPassword || '') || KIMITOOL_DEFAULT_CPA_TOKEN,
    cloudflareTempEmailBaseUrl: String(state?.cloudflareTempEmailBaseUrl || '').trim() || KIMITOOL_DEFAULT_MAIL_API_URL,
    cloudflareTempEmailAdminAuth: String(state?.cloudflareTempEmailAdminAuth || '') || KIMITOOL_DEFAULT_MAIL_ADMIN_AUTH,
    cloudflareTempEmailLookupMode: normalizeCloudflareTempEmailLookupMode(state?.cloudflareTempEmailLookupMode),
    cloudflareTempEmailDomain: normalizeCloudflareTempEmailDomain(state?.cloudflareTempEmailDomain) || KIMITOOL_ALLOWED_EMAIL_DOMAIN,
    cloudflareTempEmailDomains: normalizedDomains.length
      ? normalizedDomains
      : (KIMITOOL_ALLOWED_EMAIL_DOMAIN ? [KIMITOOL_ALLOWED_EMAIL_DOMAIN] : []),
  };
}

function getConfiguredCpaReloginAllowedEmailDomain(state = {}) {
  const activeDomain = normalizeCloudflareTempEmailDomain(state?.cloudflareTempEmailDomain);
  if (activeDomain) {
    return activeDomain;
  }
  const domains = normalizeCloudflareTempEmailDomains(state?.cloudflareTempEmailDomains);
  if (domains.length) {
    return domains[0];
  }
  return KIMITOOL_ALLOWED_EMAIL_DOMAIN;
}

function formatConfiguredEmailDomainLabel(domain = '') {
  const normalized = String(domain || '').trim().toLowerCase().replace(/^@+/, '');
  return normalized ? `@${normalized}` : '已配置域名';
}
...
function normalizePanelMode(value = '') {
  ...
  if (SESSION_TOKEN_EXPORT_PANEL_MODES.includes(normalized)) {
    return normalized;
  }
...
function isSessionTokenExportPanelMode(panelMode = '') {
  return SESSION_TOKEN_EXPORT_PANEL_MODES.includes(String(panelMode || '').trim().toLowerCase());
}

function isCpaReloginPanelMode(panelMode = '') {
...
function normalizePlusAccountAccessStrategyForState(state = {}) {
  ...
  if (isCpaReloginPanelMode(panelMode)) {
    return PLUS_ACCOUNT_ACCESS_STRATEGY_CPA_CODEX_SESSION;
  }
...
async function getState() {
  ...
  return buildStateViewWithRuntimeState(applyKimiSessionGateDefaults({
    ...DEFAULT_STATE,
    ...persistedSettings,
    ...persistedAliasState,
    ...state,
...
const HOTMAIL_VERIFICATION_TARGET_DOMAINS = new Set([
  'hotmail.com',
  'outlook.com',
  'live.com',
  'msn.com',
...
function isHotmailVerificationTargetEmail(email = '') {
  const parts = parseEmailAddressParts(email);
  if (!parts) {
    return false;
  }
  return HOTMAIL_VERIFICATION_TARGET_DOMAINS.has(parts.domain.toLowerCase());
}

function hotmailAccountMatchesVerificationTarget(account = {}, targetEmail = '') {
  const normalizedTarget = normalizeEmailAddressForMatch(targetEmail);
  if (!normalizedTarget) {
    return false;
  }
  const accountEmail = normalizeEmailAddressForMatch(account?.email);
  return accountEmail === normalizedTarget || isOutlookPlusAliasForAccount(normalizedTarget, account);
}

function findHotmailAccountByVerificationTarget(accounts, targetEmail = '') {
  return normalizeHotmailAccounts(accounts)
    .find((account) => hotmailAccountMatchesVerificationTarget(account, targetEmail)) || null;
}

function isReusableHotmailVerificationAccount(account) {
  return Boolean(account)
    && account.status === 'authorized'
    && Boolean(account.refreshToken);
}

async function resolveHotmailAccountForVerificationTarget(state = {}, pollPayload = {}) {
  const targetEmail = normalizeEmailAddressForMatch(pollPayload.targetEmail);
  const targetAccount = targetEmail
    ? findHotmailAccountByVerificationTarget(state.hotmailAccounts, targetEmail)
    : null;

  if (targetAccount) {
    if (!isReusableHotmailVerificationAccount(targetAccount)) {
      throw new Error(`Hotmail/Outlook 验证目标 ${targetEmail} 对应账号 ${targetAccount.email || targetAccount.id} 尚未授权，无法读取验证码。`);
    }
    const account = await setCurrentHotmailAccount(targetAccount.id, {
      markUsed: false,
      syncEmail: false,
    });
    return { account, targetEmail, matchedTarget: true };
  }

  if (targetEmail && isHotmailVerificationTargetEmail(targetEmail)) {
    throw new Error(`验证码页显示邮箱为 ${targetEmail}，但 Hotmail/Outlook 账号池中没有对应账号，已停止轮询以避免读取错误邮箱。`);
  }
  ...
  return { account, targetEmail, matchedTarget: false };
}
...
async function pollHotmailVerificationCode(step, state, pollPayload = {}) {
  ...
  const resolvedAccount = await resolveHotmailAccountForVerificationTarget(state, pollPayload);
...
function getPanelModeLabel(modeOrState) {
  ...
  if (mode === ACCOUNT_TOKEN_PANEL_MODE) {
    return 'accountToken';
  }
  if (mode === ACCESS_TOKEN_PANEL_MODE) {
    return 'accesToken';
  }
  if (mode === SESSION_TOKEN_BUNDLE_PANEL_MODE) {
    return 'accountToken + accesToken';
  }
  if (mode === 'sub2api') {
    return 'SUB2API';
  }
...
async function markCpaReloginQueueFromRunRecord(status, state = {}, reason = '') {
  const email = String(state?.currentCpaReloginEmail || '').trim().toLowerCase();
  if (!email || !Array.isArray(state?.cpaReloginQueue) || !state.cpaReloginQueue.length) {
    return;
  }
  const helpers = self.MultiPageCpaReloginQueue;
  if (!helpers?.markCpaReloginQueueEntry) {
    return;
  }
  const normalizedStatus = String(status || '').trim().toLowerCase();
  const queueStatus = normalizedStatus === 'success'
    ? 'success'
    : (normalizedStatus.includes('stopped') ? 'stopped' : (normalizedStatus.includes('failed') || normalizedStatus === 'failed' ? 'failed' : ''));
  if (!queueStatus) {
    return;
  }
  const queue = helpers.markCpaReloginQueueEntry(state.cpaReloginQueue, {
    allowedDomain: getConfiguredCpaReloginAllowedEmailDomain(state),
    email,
    status: queueStatus,
    reason,
    now: Date.now(),
  });
  const counts = helpers.getCpaReloginQueueCounts(queue);
  const updates = {
    cpaReloginQueue: queue,
    cpaReloginQueueCounts: counts,
    cpaReloginQueueUpdatedAt: Date.now(),
    currentCpaReloginEmail: '',
    currentCpaReloginRun: 0,
  };
  await setState(updates);
...
async function appendAndBroadcastAccountRunRecord(status, stateOverride = null, reason = '') {
  ...
  await markCpaReloginQueueFromRunRecord(resolvedStatus, state, resolvedReason);
...
function shouldUseCpaReloginQueueForAutoRun(state = {}) {
  if (!isCpaReloginPanelMode(getPanelMode(state))) {
    return false;
  }
...
function getCpaReloginQueueHelpers() {
  const helpers = self.MultiPageCpaReloginQueue;
  if (!helpers) {
    throw new Error('CPA 重登队列模块未加载。');
  }
  return helpers;
}

async function fetchCpaAuthFilesForRelogin(state = {}) {
  const api = getCpaSessionExportApi();
  const origin = api.deriveCpaManagementOrigin(state?.vpsUrl);
  const managementKey = String(state?.vpsPassword || '').trim();
  if (!managementKey) {
    throw new Error('CPA 重登队列：缺少 CPA 管理密钥。');
  }
  const payload = await api.fetchCpaManagementJson(origin, '/v0/management/auth-files', {
    method: 'GET',
    managementKey,
    timeoutMs: 30000,
  });
  ...
  const files = payload?.files || payload?.data || payload?.items || [];
  return Array.isArray(files) ? files : [];
}

async function probeCpaAuthFileForRelogin(state = {}, item = {}) {
  const authIndex = String(item?.auth_index || item?.authIndex || '').trim();
  if (!authIndex) {
    return { ...item, probe: { ok: false, error: 'missing auth_index' } };
  }
  const api = getCpaSessionExportApi();
  const origin = api.deriveCpaManagementOrigin(state?.vpsUrl);
  const managementKey = String(state?.vpsPassword || '').trim();
  ...
  const accountId = String(item?.account_id || item?.chatgpt_account_id || '').trim();
  if (accountId) {
    headers['Chatgpt-Account-Id'] = accountId;
  }
  try {
    const probe = await api.fetchCpaManagementJson(origin, '/v0/management/api-call', {
      method: 'POST',
      managementKey,
      timeoutMs: 30000,
      body: {
    ...
    return { ...item, probe: { ok: false, error: error?.message || String(error || 'probe failed') } };
  }
}

async function fetchCpaReloginMailStatus(state = {}, email = '') {
  const helpers = getCpaReloginQueueHelpers();
  const normalizedEmail = String(email || '').trim().toLowerCase();
  if (!normalizedEmail) {
    return { status: 'unknown', reason: '' };
  }
  try {
    const result = await listCloudflareTempEmailMessages(state, {
      address: normalizedEmail,
      limit: 10,
      offset: 0,
    });
    const details = await fetchCloudflareTempEmailMessageDetails(result.config, result.messages, { limit: 10 });
    return helpers.classifyMailMessages(details.length ? details : result.messages);
  } catch (error) {
    return {
      status: 'error',
      reason: error?.message || String(error || 'mail check failed'),
    };
  }
}

async function prepareCpaReloginQueueForAutoRun(options = {}) {
  const state = await getState();
  if (!shouldUseCpaReloginQueueForAutoRun(state)) {
    return { enabled: false, totalRuns: 0 };
  }

  const helpers = getCpaReloginQueueHelpers();
  const allowedDomain = getConfiguredCpaReloginAllowedEmailDomain(state);
  await addLog('CPA 重登队列：正在读取 CPA auth-files 并检测 401 账号...', 'info');
  const files = await fetchCpaAuthFilesForRelogin(state);
  const scopedFiles = files.filter((item) => {
    const email = helpers.extractCpaEmail(item);
    return email && (!allowedDomain || email.endsWith(`@${allowedDomain}`));
  });

  const inspected = [];
  for (let index = 0; index < scopedFiles.length; index += 1) {
    throwIfStopped();
    const item = scopedFiles[index];
    const preStatus = helpers.classifyCpaAuthFile(item);
    if (preStatus === 'banned') {
      inspected.push(item);
      continue;
    }
    inspected.push(await probeCpaAuthFileForRelogin(state, item));
    if ((index + 1) % 20 === 0 || index + 1 === scopedFiles.length) {
      await addLog(`CPA 重登队列：401 检测进度 ${index + 1}/${scopedFiles.length}`, 'info');
    }
  }

  const mailStatuses = {};
  const tokenInvalidItems = inspected.filter((item) => helpers.classifyCpaAuthFile(item) === 'token_invalid');
  for (let index = 0; index < tokenInvalidItems.length; index += 1) {
    throwIfStopped();
    const email = helpers.extractCpaEmail(tokenInvalidItems[index]);
    mailStatuses[email] = await fetchCpaReloginMailStatus(state, email);
    if ((index + 1) % 10 === 0 || index + 1 === tokenInvalidItems.length) {
      await addLog(`CPA 重登队列：封禁邮件检查进度 ${index + 1}/${tokenInvalidItems.length}`, 'info');
    }
  }

  let queue = helpers.buildCpaReloginQueue(inspected, {
    allowedDomain,
    mailStatuses,
    now: Date.now(),
  });
  if (typeof helpers.mergeCpaReloginQueueWithPrevious === 'function') {
    queue = helpers.mergeCpaReloginQueueWithPrevious(queue, state?.cpaReloginQueue || [], {
      allowedDomain,
    });
  }
  if (
    typeof helpers.mergeCpaReloginQueueWithRunHistory === 'function'
    && accountRunHistoryHelpers?.getPersistedAccountRunHistory
  ) {
    const runHistory = await accountRunHistoryHelpers.getPersistedAccountRunHistory();
    queue = helpers.mergeCpaReloginQueueWithRunHistory(queue, runHistory, {
      allowedDomain,
    });
  }
  const counts = helpers.getCpaReloginQueueCounts(queue);
  const runnableEmails = helpers.getRunnableCpaReloginEmails(queue);
  const updates = {
    cpaReloginQueue: queue,
    cpaReloginQueueCounts: counts,
    cpaReloginQueueSource: 'cpa-401',
    cpaReloginQueueUpdatedAt: Date.now(),
  };
  await setState(updates);
  broadcastDataUpdate(updates);
  await addLog(
    `CPA 重登队列：已生成 ${runnableEmails.length} 个待登录账号，封禁 ${counts.banned || 0} 个。`,
    runnableEmails.length ? 'ok' : 'warn'
  );
  if (!runnableEmails.length && options.throwIfEmpty !== false) {
    throw new Error(`CPA 重登队列为空：没有可登录的 ${formatConfiguredEmailDomainLabel(allowedDomain)} 401 账号。`);
  }
  return {
    enabled: true,
    totalRuns: runnableEmails.length,
    queue,
    counts,
  };
}

async function ensureCpaReloginQueueEmailReady(targetRun, totalRuns, attemptRuns) {
  const state = await getState();
  if (!shouldUseCpaReloginQueueForAutoRun(state)) {
    return '';
  }
  const helpers = getCpaReloginQueueHelpers();
  const allowedDomain = getConfiguredCpaReloginAllowedEmailDomain(state);
  const queue = helpers.normalizeCpaReloginQueue(state?.cpaReloginQueue || [], {
    allowedDomain,
  });
  if (!queue.length) {
    return '';
  }
  const lock = helpers.lockCpaReloginQueueEntry(queue, {
    allowedDomain,
    run: targetRun,
    attempt: attemptRuns,
    now: Date.now(),
  ...
  if (!lock.email) {
    throw new Error('CPA 重登队列没有剩余待登录账号。');
  }
  const counts = helpers.getCpaReloginQueueCounts(lock.queue);
  await setEmailState(lock.email, { source: 'cpa-relogin-queue' });
  await setState({
    cpaReloginQueue: lock.queue,
    cpaReloginQueueCounts: counts,
    cpaReloginQueueUpdatedAt: Date.now(),
    currentCpaReloginEmail: lock.email,
    currentCpaReloginRun: targetRun,
    signupMethod: SIGNUP_METHOD_EMAIL,
    resolvedSignupMethod: SIGNUP_METHOD_EMAIL,
    forceLoginIdentifierType: 'email',
    forceEmailLogin: true,
    accountIdentifierType: 'email',
    accountIdentifier: lock.email,
  });
  broadcastDataUpdate({
    cpaReloginQueue: lock.queue,
    cpaReloginQueueCounts: counts,
  ...
  await addLog(`=== 目标 ${targetRun}/${totalRuns} 轮：CPA 重登队列邮箱已就绪：${lock.email}（第 ${attemptRuns} 次尝试）===`, 'ok');
  return lock.email;
}

async function ensureLoginConfigEmailReady(targetRun, totalRuns, attemptRuns) {
  const currentState = await getState();
  if (shouldUseCpaReloginQueueForAutoRun(currentState)) {
    throw new Error('CPA 重登流程必须使用 CPA 401 队列，不允许回退到本地登录配置邮箱。');
  }
  const nodeIds = getAutoRunWorkflowNodeIds(currentState);
  if (!nodeIds.includes('oauth-login') || nodeIds.includes('submit-signup-email')) {
    return null;
  }
  if (!loginConfigEmailReader) {
    throw new Error('登录配置邮箱读取器未接入。');
  }

  const emails = await loginConfigEmailReader.read();
  if (!emails.length) {
    const allowedDomain = getConfiguredCpaReloginAllowedEmailDomain(currentState);
    throw new Error(`登录配置文件 ${KIMITOOL_RELOGIN_EMAIL_CONFIG_PATH} 没有可用的 ${formatConfiguredEmailDomainLabel(allowedDomain)} 邮箱。`);
  }

  const numericRun = Math.max(1, Math.floor(Number(targetRun) || 1));
  const queuedEmail = emails[numericRun - 1] || '';
  if (!queuedEmail) {
    throw new Error(`登录配置文件只有 ${emails.length} 个邮箱，无法执行第 ${numericRun} 轮。`);
  }

  await setEmailState(queuedEmail, { source: 'login-config' });
  await setState({
    signupMethod: SIGNUP_METHOD_EMAIL,
    resolvedSignupMethod: SIGNUP_METHOD_EMAIL,
    forceLoginIdentifierType: 'email',
    forceEmailLogin: true,
    accountIdentifierType: 'email',
    accountIdentifier: queuedEmail,
    currentLoginConfigEmail: queuedEmail,
    currentLoginConfigRun: numericRun,
  });
  await addLog(`=== 目标 ${numericRun}/${totalRuns} 轮：配置文件登录邮箱已就绪：${queuedEmail}（第 ${attemptRuns} 次尝试）===`, 'ok');
  return queuedEmail;
}

async function executeCpaReloginPrepareNode(state = {}) {
  const result = await prepareCpaReloginQueueForAutoRun({ throwIfEmpty: true });
  await completeNodeFromBackground(state?.nodeId || 'cpa-relogin-prepare', {
    cpaReloginQueueCounts: result.counts || {},
    cpaReloginQueueTotalRuns: result.totalRuns || 0,
  });
  return result;
}
...
const chatGptWebLoginExecutor = self.MultiPageBackgroundChatGptWebLogin?.createChatGptWebLoginExecutor({
  addLog,
  completeNodeFromBackground,
  getErrorMessage,
  getLoginAuthStateLabel,
...
const stepExecutorsByKey = {
  ...
  'session-token-export': (state) => step6Executor.executeSessionTokenExport(state),
  ...
  'chatgpt-web-login': (state) => chatGptWebLoginExecutor.executeChatGptWebLogin(state),
  'oauth-login': (state) => isCpaReloginPanelMode(getPanelMode(state))
    ? chatGptWebLoginExecutor.executeChatGptWebLogin(state)
...
const messageRouter = self.MultiPageBackgroundMessageRouter?.createMessageRouter({
  ...
  prepareCpaReloginQueueForAutoRun,
  registerTab,
  requestStop,
  probeIpProxyExit: null,
  resetState,
...
async function rerunStep7ForStep8Recovery(options = {}) {
  ...
  try {
    ...
    if (isCpaReloginPanelMode(getPanelMode(initialState))) {
      await chatGptWebLoginExecutor.executeChatGptWebLogin(loginPayload);
    } else {
      await step7Executor.executeStep7(loginPayload);
    }
  } catch (err) {
    const latestState = await getState();
    if (isStopError(err)) {
