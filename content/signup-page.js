...
const SIGNUP_PAGE_NODE_HANDLERS = Object.freeze({
  ...
  'chatgpt-web-login': (payload) => step6_login(payload),
...
const COOKIE_CONSENT_ACTION_PATTERN = /accept(?:\s+all)?|allow(?:\s+all)?|reject(?:\s+(?:all|non[-\s]*essential|optional))?|decline(?:\s+(?:all|optional|non[-\s]*essential))?|agree|i\s+agree|got\s+it|confirm(?:\s+choices)?|save\s+(?:choices|preferences)|同意|接受|允许|拒绝|确认|保存(?:选择|偏好)|好的|知道了/i;
const COOKIE_CONSENT_CONTEXT_PATTERN = /cookie|cookies|privacy|consent|preferences|personalized?|隐私|同意|授权|偏好|Cookie/i;
...
function isChatGptHomeUrl(rawUrl = location.href) {
  try {
...
function findCookieConsentTrigger() {
  ...
  const matches = candidates.filter((el) => {
    ...
    if (!text || !COOKIE_CONSENT_ACTION_PATTERN.test(text)) {
      return false;
    }
    if (/^(?:continue|继续)$/i.test(text)) {
      const contextText = [
        el.closest?.('[role="dialog"], [aria-modal="true"], section, aside, form, div')?.textContent,
        document.body?.innerText,
      ].filter(Boolean).join(' ').replace(/\s+/g, ' ').trim();
      return COOKIE_CONSENT_CONTEXT_PATTERN.test(contextText);
    }
    return true;
  });
  if (!matches.length) {
  ...
  return matches.find((el) => /accept(?:\s+all)?|reject\s+non[-\s]*essential/i.test(getActionText(el)))
    || matches[0]
    || null;
}

async function dismissCookieConsentIfPresent(options = {}) {
  const trigger = findCookieConsentTrigger();
  if (!trigger) {
    return false;
  }
  const visibleStep = Math.floor(Number(options?.visibleStep) || 0) || 7;
  log(`步骤 ${visibleStep}：检测到 cookie/隐私确认框，正在点击 "${getActionText(trigger).slice(0, 60)}"...`, 'info', { step: visibleStep, stepKey: 'oauth-login' });
  simulateClick(trigger);
  const startedAt = Date.now();
  const timeoutMs = Math.max(800, Math.floor(Number(options?.timeoutMs) || 4000));
  while (Date.now() - startedAt < timeoutMs) {
    await sleep(200);
    if (!findCookieConsentTrigger()) {
      return true;
    }
  }
  return true;
...
async function completeSignupProfileAfterVerification(step, signupProfile = {}, baseResult = {}) {
  if (!signupProfile?.firstName || !signupProfile?.lastName) {
    throw new Error(`步骤 ${step}：验证码通过后进入资料页，但未提供姓名/年龄资料。`);
  }
  ...
  const profileCompletion = step === 8
    ? await waitForPhoneVerificationProfileCompletion()
    : {};
  const result = {
    ...baseResult,
    ...profileCompletion,
    success: true,
    addPhonePage: Boolean(profileCompletion.returnedToAddPhone),
    profileCompleted: true,
    profilePage: true,
    url: profileCompletion.url || baseResult.url || location.href,
  };
  if (profileCompletion.consentReady || profileCompletion.returnedToAddPhone) {
    delete result.assumed;
  }
  return result;
}

function getStep5NameInputSelector() {
  return [
    'input[name="name"]',
    'input[autocomplete="name"]',
    'input[placeholder*="全名"]',
...
function getStep5AgeInputSelector() {
  return [
    'input[name="age"]',
    'input[placeholder*="年龄"]',
    'input[aria-label*="年龄"]',
...
function resolveLoginEntryHref(el) {
  const rawHref = String(el?.href || el?.getAttribute?.('href') || '').trim();
  if (!rawHref) {
    return '';
  }
  try {
...
function isLikelyLoginEntryHref(el) {
  const href = resolveLoginEntryHref(el);
  if (!href) {
    return false;
  }
  try {
    const parsed = new URL(href);
    return /\/(?:auth\/)?(?:log-?in|sign-?in|login)(?:[/?#]|$)/i.test(parsed.pathname);
  } catch {
    return false;
  }
}
...
function findLoginEntryTrigger() {
  ...
  if (isChatGptHomeUrl()) {
    const hrefLogin = candidates.find((el) => isLikelyLoginEntryHref(el));
  ...
  const chatGptHomeLogin = isChatGptHomeUrl()
    ? candidates.find((el) => {
        const text = getActionText(el);
        if (!text || LOGIN_CODE_ONLY_ACTION_PATTERN.test(text) || LOGIN_EXTERNAL_IDP_PATTERN.test(text)) return false;
        return /(?:^|\b)(?:log\s*in|sign\s*in)(?:\b|$)|登录|登陆/i.test(text);
      })
    : null;
  if (chatGptHomeLogin) return chatGptHomeLogin;
...
function inspectLoginAuthState() {
  ...
  const signupProfilePage = isStep5Ready() || isSignupProfilePageUrl();
  const baseState = {
    state: 'unknown',
    url: location.href,
...
function serializeLoginAuthState(snapshot) {
  return {
    ...
    signupProfilePage: Boolean(snapshot?.signupProfilePage),
  };
}
...
async function waitForKnownLoginAuthState(timeout = 15000) {
  const start = Date.now();
  await dismissCookieConsentIfPresent({ visibleStep: 7 }).catch(() => false);
...
async function waitForVerificationSubmitOutcome(step, timeout, options = {}) {
  ...
  while (Date.now() - start < resolvedTimeout) {
    ...
    if (step === 8 && (isStep5Ready() || isSignupProfilePageUrl())) {
      return await completeSignupProfileAfterVerification(step, options?.signupProfile || {}, {
        url: location.href,
      });
    }
...
async function fillVerificationCode(step, payload) {
  ...
  if (step === 8) {
    if (isStep5Ready() || isSignupProfilePageUrl()) {
      return await completeSignupProfileAfterVerification(step, signupProfile || {}, {
        assumed: true,
        alreadyAdvanced: true,
        url: location.href,
      });
...
async function step6OpenLoginEntry(payload, snapshot) {
  ...
  await dismissCookieConsentIfPresent({ visibleStep }).catch(() => false);
  ...
  const triggerHref = resolveLoginEntryHref(trigger);
  ...
  let nextSnapshot = await waitForLoginEntryOpenTransition();

  if (
    (nextSnapshot.state === 'unknown' || nextSnapshot.state === 'entry_page')
    && triggerHref
    && isChatGptHomeUrl()
  ) {
    log(`登录入口点击后仍停留在首页，改用链接跳转：${triggerHref}`, 'warn', { step: visibleStep, stepKey: 'oauth-login' });
    location.assign(triggerHref);
    nextSnapshot = await waitForLoginEntryOpenTransition(15000);
  }
...
function isCombinedSignupVerificationProfilePage() {
  ...
  const nameInput = document.querySelector(getStep5NameInputSelector());
  if (!nameInput || !isVisibleElement(nameInput)) {
    return false;
  }

  const ageInput = document.querySelector(getStep5AgeInputSelector());
...
async function step5_fillNameBirthday(payload) {
  const { firstName, lastName, age, year, month, day, prefillOnly = false, reportCompletion = true } = payload;
  ...
  let nameInput = null;
  try {
    nameInput = await waitForElement(getStep5NameInputSelector(), 10000);
  } catch {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    ageInput = document.querySelector(getStep5AgeInputSelector());
