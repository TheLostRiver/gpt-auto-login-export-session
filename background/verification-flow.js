      ...
      while (Date.now() - startedAt < timeoutMs) {
        ...
        try {
          const signupTabId = typeof getTabId === 'function'
            ...
            : null;
          if (signupTabId && chrome?.tabs?.get) {
      ...
      if (typeof sendToContentScriptResilient === 'function' && !shouldAvoidReplaySubmit) {
        try {
          ...
          if (step === 8 && isRetryableVerificationTransportError(err)) {
            ...
            if (fallback.success) {
              if (fallback.profilePage) {
                await addLog('验证码提交后通信中断，但页面已进入资料页并已触发资料填写，按提交成功继续。', 'warn', {
                  step: completionStep,
                  stepKey: 'fetch-login-code',
                });
              } else if (fallback.addPhonePage) {
                await addLog('验证码提交后通信中断，但页面已进入手机号验证页，按提交成功继续。', 'warn', {
                  step: completionStep,
                  stepKey: 'fetch-login-code',
                });
              } else if (fallback.reason === 'chatgpt_home') {
                await addLog('验证码提交后通信中断，但页面已进入 ChatGPT 首页，按登录成功继续。', 'warn', {
                  step: completionStep,
                  stepKey: 'fetch-login-code',
                });
              } else {
              ...
              return {
                ...
                profilePage: Boolean(fallback.profilePage),
                url: fallback.url || '',
              };
            }
      ...
      if (mail.provider === HOTMAIL_PROVIDER) {
        ...
        for (let attempt = 1; attempt <= maxSubmitAttempts; attempt++) {
          const pollOptions = {
            ...
            resendIntervalMs,
            lastResendAt,
            targetEmail: options.targetEmail || (step === 4
              ? state.email
              : (String(state?.step8VerificationTargetEmail || '').trim() || state.email)),
