(function attachBackgroundLocalCliProxyApi(root, factory) {
  ...
  async function buildSessionTokenArtifacts(options = {}) {
    const exportMode = normalizeString(options.exportMode || options.panelMode).toLowerCase();
    const includeAccountToken = exportMode === 'account-token' || exportMode === 'session-token-bundle';
    const includeAccessToken = exportMode === 'access-token' || exportMode === 'session-token-bundle';
    if (!includeAccountToken && !includeAccessToken) {
      throw new Error('生成账号 token 导出文件失败：未知导出模式。');
    }

    const pluginDir = normalizeString(options.pluginDir);
    if (!pluginDir) {
      throw new Error('生成账号 token 导出文件失败：缺少 pluginDir。');
    }

    const sourceSession = options.session && typeof options.session === 'object' && !Array.isArray(options.session)
      ? options.session
      : {};
    const accessToken = normalizeString(options.accessToken || options.access_token || sourceSession.accessToken);
    if (!accessToken) {
      throw new Error('生成账号 token 导出文件失败：缺少 accessToken。');
    }
    ...
    const fileSegment = sanitizeFileSegment(email || accountId, 'chatgpt-session');
    ...
    if (includeAccountToken) {
      const fileName = `accountToken-${fileSegment}.json`;
      artifacts.push({
        provider: 'codex',
        kind: 'account-token',
        fileName,
        ...
        relativeAuthDir,
        authJson: session,
        jsonText: `${JSON.stringify(session, null, 2)}\n`,
        mimeType: 'application/json;charset=utf-8',
      });
    }

    if (includeAccessToken) {
      const fileName = `accesToken-${fileSegment}.txt`;
      artifacts.push({
        provider: 'codex',
        kind: 'access-token',
        fileName,
