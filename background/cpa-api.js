(function attachBackgroundCpaApi(root, factory) {
  ...
  const DEFAULT_ALLOWED_CPA_SESSION_EMAIL_DOMAIN = '';
  ...
  function createCpaApi(deps = {}) {
    ...
    function normalizeAllowedEmailDomains(state = {}) {
      const values = [];
      ...
      pushDomain(state?.cloudflareTempEmailDomain);
      for (const domain of Array.isArray(state?.cloudflareTempEmailDomains) ? state.cloudflareTempEmailDomains : []) {
        pushDomain(domain);
      }
      pushDomain(DEFAULT_ALLOWED_CPA_SESSION_EMAIL_DOMAIN);
      return values;
    }

    function formatAllowedEmailDomainsLabel(domains = []) {
      return domains.length ? domains.map((domain) => `@${domain}`).join(' / ') : '已配置邮箱域名';
    }

    function isAllowedCpaSessionEmail(email = '', state = {}) {
      const normalizedEmail = normalizeEmailValue(email).toLowerCase();
      if (!normalizedEmail) {
        return false;
      }
      const domains = normalizeAllowedEmailDomains(state);
      return !domains.length || domains.some((domain) => normalizedEmail.endsWith(`@${domain}`));
    }
    ...
    function buildCpaSessionAuthJson(state = {}, options = {}) {
      ...
      if (!isAllowedCpaSessionEmail(email, state)) {
        const allowedDomains = normalizeAllowedEmailDomains(state);
        throw new Error(`当前 ChatGPT 登录账号不是 ${formatAllowedEmailDomainsLabel(allowedDomains)} 邮箱，已拒绝导入 CPA。`);
      }
