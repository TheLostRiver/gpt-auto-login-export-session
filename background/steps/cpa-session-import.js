(function attachBackgroundCpaSessionImport(root, factory) {
  ...
  function createCpaSessionImportExecutor(deps = {}) {
    ...
    function isReadableChatGptSessionUrl(url = '') {
      try {
    ...
    async function prepareReadableSessionTab(tabId, visibleStep) {
      const tab = await getResolvedSessionTab(tabId, visibleStep);
      if (isReadableChatGptSessionUrl(tab.url)) {
        return tab;
      }
      if (!chrome?.tabs?.update) {
        throw new Error(`步骤 ${visibleStep}：当前标签页不在 ChatGPT 页面，且无法自动跳转读取会话。`);
      }
      await addStepLog(visibleStep, '当前登录页不是 ChatGPT 会话页，正在打开 ChatGPT 读取当前登录 SESSION...', 'info');
      ...
      return updatedTab?.id ? updatedTab : { ...tab, url: CHATGPT_SESSION_EXPORT_URL };
    }
    ...
    async function executeCpaSessionImport(state = {}) {
      ...
      const result = await runSessionImportWithRetries(async (attempt) => {
        ...
        const tab = await prepareReadableSessionTab(tabId, visibleStep);
        if (chrome?.tabs?.update) {
