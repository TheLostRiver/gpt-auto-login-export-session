(function attachBackgroundStep8(root, factory) {
  ...
  function createStep8Executor(deps = {}) {
    ...
    function buildSignupProfileForVerificationStep() {
      ...
      const age = typeof generateRandomAge === 'function' ? generateRandomAge() : null;
      const birthday = typeof generateRandomBirthday === 'function'
        ? generateRandomBirthday(age ? { age } : undefined)
        : null;
      ...
      return {
        ...
        age: age || birthday?.age || (birthday.year ? new Date().getFullYear() - Number(birthday.year) : null),
        year: birthday.year,
    ...
    function getAuthLoginStepForVisibleStep(visibleStep) {
      const numericStep = Math.floor(Number(visibleStep) || 0);
      if (numericStep > 0 && numericStep < 7) {
        return Math.max(1, numericStep - 1);
      }
