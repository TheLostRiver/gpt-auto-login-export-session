(function attachBackgroundStep1(root, factory) {
  ...
  function shouldCloseStep1Tab(tab = {}) {
    ...
    try {
      ...
      return STEP1_CLOSE_TAB_HOSTS.includes(host);
    } catch {
      return false;
    }
  }
  ...
  function createStep1Executor(deps = {}) {
    ...
    async function clearOpenAiCookiesBeforeStep1() {
      ...
      if (chromeApi.browsingData?.remove) {
        try {
          ...
          await addLog(`步骤 1：browsingData 清理站点数据失败：${getStep1ErrorMessage(error)}`, 'warn');
        }
      }
    ...
    async function closeOpenAiTabsBeforeStep1() {
      ...
      const tabIds = (Array.isArray(tabs) ? tabs : [])
        .filter((tab) => Number.isInteger(tab?.id) && shouldCloseStep1Tab(tab))
        .map((tab) => tab.id);
      if (!tabIds.length) {
        return;
      }
      await chromeApi.tabs.remove(tabIds).catch(() => {});
      await addLog(`步骤 1：已关闭 ${tabIds.length} 个旧 ChatGPT / Auth 标签页。`, 'info');
    }

    async function executeStep1() {
      await closeOpenAiTabsBeforeStep1();
