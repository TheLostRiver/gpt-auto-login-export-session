(function attachBackgroundStep6(root, factory) {
  ...
  const SESSION_TOKEN_EXPORT_NODE_ID = 'session-token-export';
  ...
  function createStep6Executor(deps = {}) {
    ...
    function isSessionTokenExportMode(state = {}) {
      return SESSION_TOKEN_EXPORT_PANEL_MODES.includes(normalizeString(getPanelMode(state)));
    }
    ...
    async function exportSessionTokenFiles(state = {}, options = {}) {
      const visibleStep = Math.max(1, Math.floor(Number(options.visibleStep) || 5));
      const helperBaseUrl = normalizeHotmailLocalBaseUrl(state.hotmailLocalBaseUrl);
      const pluginDir = normalizeString(state.localCpaJsonPluginDir);
      if (!helperBaseUrl) {
        throw new Error('尚未配置 Hotmail 本地助手地址，请先在侧边栏填写。');
      ...
      const bundle = await api.buildSessionTokenArtifacts({
        exportMode: normalizeString(getPanelMode(state)),
        pluginDir,
        relativeAuthDir: state.localCpaJsonRelativeAuthDir,
        session: sessionResult?.session,
        accessToken: sessionResult?.accessToken,
      ...
      for (const artifact of bundle.artifacts || []) {
        savedArtifacts.push(await saveLocalCpaJsonArtifactViaHelper(helperBaseUrl, artifact));
      }

      const filePaths = savedArtifacts
        .map((artifact) => artifact.filePath)
        .filter(Boolean);
      const verifiedStatus = `账号 Token 文件已导出：${filePaths.join('，')}`;
      await addLog(`步骤 ${visibleStep}：${verifiedStatus}`, 'ok');
      return {
        verifiedStatus,
        sessionTokenExportMode: bundle.exportMode,
        sessionTokenExportFilePaths: filePaths,
      };
    }
    ...
    async function executeSessionTokenExport(state = {}) {
      if (!isSessionTokenExportMode(state)) {
        throw new Error('当前不是账号 Token 导出模式，不能执行账号 Token 导出节点。');
      }
      await addLog('步骤 5：登录完成，正在导出账号 Token 文件...', 'info');
      const completionPayload = await exportSessionTokenFiles(state, { visibleStep: 5 });
      await completeNodeFromBackground(SESSION_TOKEN_EXPORT_NODE_ID, completionPayload);
    }

    return {
      executeLocalCpaJsonNoRtExport,
      executeSessionTokenExport,
      executeStep6,
    };
  }
