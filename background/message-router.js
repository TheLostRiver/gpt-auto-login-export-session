  ...
  function createMessageRouter(deps = {}) {
    ...
    async function handleMessage(rawMessage, sender) {
      ...
      switch (message.type) {
        ...
        case 'PREPARE_CPA_RELOGIN_QUEUE': {
          if (typeof prepareCpaReloginQueueForAutoRun !== 'function') {
            throw new Error('CPA 重登队列能力未接入。');
          }
          const result = await prepareCpaReloginQueueForAutoRun({ throwIfEmpty: false });
        ...
        case 'AUTO_RUN': {
          ...
          let totalRuns = normalizeRunCount(message.payload?.totalRuns || 1);
          if (typeof prepareCpaReloginQueueForAutoRun === 'function') {
            const queuePrep = await prepareCpaReloginQueueForAutoRun({ throwIfEmpty: true });
            if (queuePrep?.enabled) {
              totalRuns = normalizeRunCount(queuePrep.totalRuns || 1);
            }
          }
        ...
        case 'SAVE_SETTING': {
          ...
          const plusAccountAccessStrategyChanged = Object.prototype.hasOwnProperty.call(updates, 'plusAccountAccessStrategy')
            && String(currentState?.plusAccountAccessStrategy || '').trim() !== String(updates.plusAccountAccessStrategy || '').trim();
          const signupMethodChanged = Object.prototype.hasOwnProperty.call(updates, 'signupMethod')
            && normalizeSignupMethod(currentState?.signupMethod) !== normalizeSignupMethod(updates.signupMethod);
